...
def get_refinery_demand(ref_id):
    ...
    demand_kg_day = np.round(demand_sfc*SCF_TO_KGH2,2)
...
def main():
  ...
  for ref_id in ref_ids:
    ...
    try:
      ...
      total_df = pd.concat([total_df, pd.DataFrame.from_dict(data=result_ref)])
      total_df.to_csv('results_ref_deployement.csv', header=True, index=False)
    except ValueError:
